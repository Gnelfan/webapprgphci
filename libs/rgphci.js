...
const text = "RECENSEMENT GENERAL DE LA POPULATION EN COTE D'IVOIRE";
...
const links = [
  { label: "1975", url: "cartes/rgphci75.html" },
  { label: "1998", url: "cartes/rgphci98.html" },
  { label: "2014", url: "cartes/rgphci14.html" },
];
...
// ajouter un nouveau bouton pour le controle des cartes
...
//Ajout fichier geoJSON population 1975 
const geojsonPop =  L.geoJSON(pop1975JSON, {
...
//Ajout fichier geoJSON ReGion 2013
...
//Ajout fichier geoJSON DPT 2013
...
const overlayLayers = {
    "Population en 1975": geojsonPop,
    "Régions 2013": geojsonLayer2,
    "Déprtements 2013": geojsonLayer3,
    "Sous-préfectures 2013": geojsonLayer4,
...
// Fonction pour masquer ou afficher les couches GeoJSON en fonction du niveau de zoom
function updateGeoJSONLayers() {
  const currentZoom = map.getZoom();
  if (currentZoom <= 8) {
    // Niveau de zoom faible : masquer toutes les couches GeoJSON voulues
    map.removeLayer(geojsonLayer1);
    map.removeLayer(geojsonLayer2);
    map.removeLayer(geojsonLayer3);
    // Ajoutez d'autres couches que vous ne voulez pas masquer ici
    map.addLayer(geojsonLayer4);
  } else if (currentZoom > 8 && currentZoom <= 10) {
    // Niveau de zoom moyen : afficher quelques couches GeoJSON
    map.addLayer(geojsonLayer3);
    // Masquer d'autres couches
    //map.removeLayer(geojsonLayer4);
    // Ajoutez d'autres couches que vous voulez afficher ici
  } else {
    // Niveau de zoom élevé : afficher toutes les couches GeoJSON voulues
...
// Gérer l'événement de changement de niveau de zoom pour mettre à jour les couches GeoJSON
map.on('zoom', function () {
  updateGeoJSONLayers();
});

// Assurez-vous de mettre à jour les couches GeoJSON au chargement initial de la carte
updateGeoJSONLayers();
